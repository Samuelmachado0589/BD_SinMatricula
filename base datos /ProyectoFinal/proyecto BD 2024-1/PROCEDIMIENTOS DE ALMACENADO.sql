...
@ID_Comentario int 
AS 
IF @AUX = 'I' AND NOT EXISTS (SELECT * FROM Comentarios where ID_Comentario = @ID_Comentario)
BEGIN 
INSERT INTO Comentarios(ID_Comentario,ID_Reserva, Comentario, Fecha_Comentario) 
VALUES (@ID_Comentario,@ID_Reserva, @Comentario, @Fecha_Comentario); 
END 
IF @AUX = 'U' 
BEGIN 
UPDATE Comentarios 
...
AS 
IF @AUX = 'I' AND NOT EXISTS (SELECT * FROM Calificacion  where ID_Calificacion = @ID_Calificacion)
BEGIN 
INSERT INTO Calificacion(ID_Calificacion, ID_Reserva, Calificacion, Fecha_Calificacion)
VALUES (@ID_Calificacion,@ID_Reserva, @Calificacion, @Fecha_Calificacion); 
END 
IF @AUX = 'U' 
BEGIN 
UPDATE Calificacion 
...
AS 
IF @AUX = 'I'  AND NOT EXISTS (SELECT * FROM Cliente  where ID_Clientes = @ID_Cliente  ) AND NOT EXISTS (SELECT * FROM Cliente  where  Documento = @Documento )
BEGIN 
INSERT INTO Cliente (Nombre, Apellido, Telefono, Email, Documento,ID_Clientes)
VALUES (@Nombre,@Apellido , @Telefono, @Email, @Documento,@ID_Cliente);
END 
IF @AUX = 'U' AND NOT EXISTS (SELECT * FROM Cliente  where ID_Clientes = @ID_Cliente  ) AND NOT EXISTS (SELECT * FROM Cliente  where  Documento = @Documento )
BEGIN 
UPDATE Cliente 
SET Nombre = @Nombre, Apellido= @Apellido, Telefono = @Telefono ,Email = @Email,Documento = @Documento,ID_Clientes = @ID_Cliente 
WHERE ID_Clientes = @ID_Cliente 
END 
IF @AUX = 'D' 
BEGIN 
delete Cliente where  ID_Clientes = @ID_Cliente
END
